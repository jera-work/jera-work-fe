...
export class QuestionAnswerComponent implements OnInit {
  ...
  ngOnInit(): void {
    firstValueFrom(this.route.paramMap).then((res) => {
      ...
      const candidateCodeAuth = this.authService.getProfile().candidateCode;

      if (candidateCodeAuth == candidateCode) {
        if (jobId) {
          firstValueFrom(this.questionService.getQuestions(jobId)).then(
            (res) => {
              this.questions = res;

              for (let i = 0; i < this.questions.length; i++) {
                this.questionAnswerReq.push(
                  this.fb.group({
                    jobVacancyId: [jobId],
  ...
  submit() {
    if (this.questionAnswerInsertReqDto.valid) {
      this.loading = true;
      this.questionAnswerService
        .insertAnswer(
          this.questionAnswerInsertReqDto.get('answerReq')?.getRawValue()
        )
        .subscribe((result) => {
          localStorage.clear();
